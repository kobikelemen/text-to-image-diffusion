...
class DDPM(nn.Module):
    def __init__(self, nn_model, betas, n_T, device, drop_prob=0.1):
        ...
        params = sum(p.numel() for p in self.nn_model.parameters() if p.requires_grad)
    ...
    def forward(self, x, c, text_embs):
        """
        ...
        context_mask_text[:, :, :] = context_mask[:, None, None]
    ...
    def sample(self, n_sample, size, device, text_embs, guide_w = 0.0):
        ...
        text_embs = text_embs.repeat(2,1,1)
        context_mask_text = context_mask_text.repeat(2,1,1)
